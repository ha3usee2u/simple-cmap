...
void rb_insert_fixup(map_t* map, rb_node_t* node) {
    while (node->parent->color == RED) {
        ...
        rb_node_t* uncle = is_left ? node->parent->parent->right : node->parent->parent->left;
        ...
        if (uncle->color == RED) {
            ...
            node->parent->color = BLACK;
            node->parent->parent->color = RED;
            is_left ? rotate_right(map, node->parent->parent) : rotate_left(map, node->parent->parent);
        }
    }
